 ...
 * 
 * @details 
 * Holds the proportional gain, integral gain, derivative gain, and upper and lower clamp bounds 
 ...
 */
 ...
 /**
  * @brief Sets initial speedPID struct values for kp, ki, kd, integralMin, integralMax, outputMin, and outputMax.
  * 
  * @details 
  * Initialized kp to 0.2, ki to 0.2, kd to 0.2, integralMin to -100.0, integralMax to 100.0, outputMin to -100.0, 
  * zeroes the integral and previousRPM values, and outputMax to 100.0. Function is called once at boot.
  * 
  * @param spedPID Passes a pointer to the speedPID struct with the PID_t type.
  */
 void PID_Initialize(PID_t *speedPID);

 /**
  * @brief Resets the integral and previousRPM values to zero.
  * 
  * @details 
  * Sets both integral and previousRPM speedPID struct values to 0. These values are included in a separate function so that they can be 

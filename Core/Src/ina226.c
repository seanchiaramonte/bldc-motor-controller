...
void INA226_Initialize(void)
{
    ...
    if (status != HAL_OK)
    {
        printf("Calibration Write Error:%d", status); // Indicates write error (especially useful since no return value is used)
    }
    ...
    if (status != HAL_OK)
    {
        printf("Configuration Write Error:%d", status);
    }
}